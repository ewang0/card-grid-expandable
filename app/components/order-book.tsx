...
const OrderRow = memo(
  ({ order, isAsk }: { order: OrderData; isAsk: boolean }) => {
    return (
      <div className="relative grid grid-cols-3 items-center px-4 py-1 text-xs">
        <motion.div
          className={`absolute inset-0 ${
            isAsk ? "bg-rose-900/30" : "bg-teal-900/30"
          } z-0`}
          initial={{ width: 0 }}
          animate={{
        ...
        <div
          className={`relative z-10 ${
            isAsk ? "text-rose-400" : "text-teal-400"
          }`}
        >
          {order.price}
        </div>
...
export default function OrderBook() {
    ...
    setAsks((prevAsks) => {
      const newAsks = prevAsks.map((ask) => {
        ...
        return {
          ...
          percentage: Math.min(
            100,
            Math.max(10, ask.percentage + (Math.random() * 30 - 15))
          ),
        };
      });
      ...
      const maxTotal = Math.max(
        ...newAsks.map((ask) => Number(ask.total.replace(/[$,]/g, "")))
      );
    ...
    setBids((prevBids) => {
      const newBids = prevBids.map((bid) => {
        ...
        return {
          ...
          percentage: Math.min(
            100,
            Math.max(10, bid.percentage + (Math.random() * 30 - 15))
          ),
        };
      });

      const maxTotal = Math.max(
        ...newBids.map((bid) => Number(bid.total.replace(/[$,]/g, "")))
      );
  ...
  const OrderBookHeader = useMemo(
    () => (
      <div className="sticky top-0 z-10 grid grid-cols-3 px-4 py-1.5 text-[10px] text-slate-400 bg-neutral-900">
        <div>PRICE</div>
  ...
  const PriceInfo = useMemo(
    () => (
      <div className="sticky z-10 grid grid-cols-3 bg-neutral-800/80 px-4 py-1.5 text-xs">
  ...
  return (
    <div className="w-full max-w-md rounded-lg border border-neutral-800 bg-neutral-800 text-slate-300">
      <div
      ...
      >
        <div className="border-b border-neutral-900">{TabButtons}</div>

        {OrderBookHeader}

        <div>
          <div className="space-y-px bg-neutral-900">
            {/* Asks (Sell orders) */}
